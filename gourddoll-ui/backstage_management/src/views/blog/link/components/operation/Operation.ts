...
import { modelRef, rulesRef, resetFields } from "./linkInitData";
  ...
  setup(props: any, context: SetupContext) {
    ...
    function load({ type, id }: any) {
      ...
      if (currentOperation.value == operationTypeEnum.edit) {
        linkController.getLinkById(id).then((data) => {
          Object.assign(modelRef, data);
        });
      }
    }
    Emitter.on("changeOperation", load, moduleEnum.link);
    ...
    function onSubmit() {
      validate().then(() => {
        ...
        if (currentOperation.value == operationTypeEnum.add) {
          save = linkController.add(toRaw(modelRef));
        } else if (currentOperation.value == operationTypeEnum.edit) {
          save = linkController.edit(toRaw(modelRef));
        }
        save.then(() => {
