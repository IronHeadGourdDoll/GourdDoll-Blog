...
import { modelRef, rulesRef, resetFields } from "./categoryInitData";
  ...
  setup(props: any, context: SetupContext) {
    ...
    function load({ type, id }: any) {
      ...
      if (currentOperation.value == operationTypeEnum.edit) {
        categoryController.getCategoryById(id).then((data) => {
          Object.assign(modelRef, data);
        });
      }
    }
    Emitter.on("changeOperation", load, moduleEnum.category);
    ...
    function onSubmit() {
      validate().then(() => {
        ...
        if (currentOperation.value == operationTypeEnum.add) {
          save = categoryController.add(toRaw(modelRef));
        } else if (currentOperation.value == operationTypeEnum.edit) {
          save = categoryController.edit(toRaw(modelRef));
        }
        save.then(() => {
